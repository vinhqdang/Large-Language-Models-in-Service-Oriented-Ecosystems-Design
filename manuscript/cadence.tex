...
The evaluation above compares CADENCE only against our own baseline re-implementations. To add a genuine external reference point, we identify our three scaled-run held-out items (matched by title, unique per repository) inside the ``Context Matters'' replication package's own results, which already reports real generations and scores from four frontier models --- Gemini-2.5-Pro, GLM-4.6, Qwen3-235B, and Gemma3-4B --- under its own \texttt{Baseline} (title-only, no context) and \texttt{RAG\_Based} ($k=3$) generation strategies, using the same metric families we use (HuggingFace \texttt{evaluate}'s ROUGE, METEOR, and BERTScore; its BLEU implementation reports on a 0--1 scale, multiplied by 100 in Table~\ref{tab:published} for consistency with our own sacrebleu-based BLEU elsewhere). \texttt{scripts/extract\_published\_baseline\_comparison.py} performs this exact-item matching and is included in the released artifact.
