...
\begin{axis}[
    ...
    height=4.3cm,
    ylabel={Score},
    symbolic x coords={zero-shot,retrieval-only,no-solver,cadence-full},
    xtick=data,
    x tick label style={font=\scriptsize},
    ymin=0, ymax=0.95,
    legend style={font=\scriptsize, at={(0.5,1.18)}, anchor=north, legend columns=-1, /tikz/every even column/.append style={column sep=6pt}},
    enlarge x limits=0.22,
    axis lines*=left,
    tick label style={font=\scriptsize},
]
\addplot coordinates {(zero-shot,0.810) (retrieval-only,0.819) (no-solver,0.805) (cadence-full,0.801)};
\addplot coordinates {(zero-shot,0.220) (retrieval-only,0.216) (no-solver,0.143) (cadence-full,0.138)};
\addplot coordinates {(zero-shot,0.244) (retrieval-only,0.250) (no-solver,0.111) (cadence-full,0.102)};
\legend{BERTScore F1, ROUGE-1 F1, METEOR}
\end{axis}
\end{tikzpicture}
\caption{Pilot run (Table~\ref{tab:pilot}): BERTScore F1 stays flat across all four systems while ROUGE-1 and METEOR drop sharply for the two deliberation-based systems (\texttt{no-solver} = \texttt{multiagent\_no\_solver}) --- the length/register mismatch this section discusses, not a quality gap.\label{fig:metric-asymmetry}}
\end{figure} We believe this asymmetry is not specific to CADENCE: any structured, multi-stage LLM pipeline whose intermediate representation is deliberately concise --- for parseability, for solver-compatibility, or for any reason other than matching a free-text reference's length --- will be penalized by surface n-gram metrics relative to a single-pass system prompted for open-ended prose, independent of whether the structured pipeline's underlying decision is better, worse, or equivalent. Evaluations of future solver-verified or otherwise structured-output LLM pipelines should report a semantic-similarity metric alongside, not instead of, surface overlap metrics for exactly this reason.
