...
This run also adds \texttt{cadence\_no\_critique} to the comparison, isolating self-critique's marginal contribution for the first time in this evaluation. At $B=5$, \texttt{cadence\_full} scores higher than \texttt{cadence\_no\_critique} on all four generation-quality metrics, but by a systematically larger margin on the three surface n-gram metrics (BLEU 1.04~vs.~0.44, ROUGE-1 0.198~vs.~0.174, METEOR 0.100~vs.~0.092) than on BERTScore (0.806~vs.~0.797, a 0.009 gap comparable in order of magnitude to the $\sim$0.018 BERTScore spread we separately observe across four structurally different systems in the pilot run, Table~\ref{tab:metric-range}, and so not distinguishable from noise on its own); at $B=2$, it scores higher on the same three n-gram metrics, with BERTScore essentially tied in the opposite direction (0.802~vs.~0.805). We therefore read the positive signal below as resting on BLEU, ROUGE-1, and METEOR's consistent agreement across both budgets, not on BERTScore, whose small differences here are not distinguishable from sampling noise given its own established spread. This is the first ablation comparison in this evaluation where the fuller pipeline wins consistently on the n-gram metrics rather than being roughly flat or mixed against a reduced version --- a genuine, if modest and small-sample, positive signal that Stage~4's self-critique finalization pass produces text scoring better against the held-out reference than the pre-critique candidate, consistent with (though not proof of) the finalization pass doing more than repackaging Stage~3's output. Both systems show the same 0\% constraint-satisfaction rate at both budgets, as expected: feasibility is decided entirely within Stage~3, before Stage~4 ever runs.
...
The evaluation above compares CADENCE only against our own baseline re-implementations. To add a genuine external reference point, we identify our three scaled-run held-out items (matched by title, unique per repository) inside the ``Context Matters'' replication package's~\cite{contextmatters-zenodo} own \texttt{Generated\_ADRs} archive, which records real generations from four frontier models --- Gemini-2.5-Pro~\cite{gemini25} (proprietary; parameter count undisclosed), GLM-4.6~\cite{glm45} ($\sim$357B total parameters, Mixture-of-Experts; per-token active-parameter count not officially disclosed for this specific release), Qwen3-235B~\cite{qwen3} (235B total, 22B active, Mixture-of-Experts), and Gemma3-4B~\cite{gemma3} (4B, dense) --- under its own \texttt{Baseline} (title-only, no context) and \texttt{RAG\_Based} ($k=3$) generation strategies, against the same held-out ground truth we use. We deliberately do \emph{not} use the package's own precomputed scores for these items: a close reading of its evaluation script found that it computes BLEU and METEOR with the prediction and reference roles reversed relative to standard usage and to our own metrics code, and computes ROUGE-1 without a stemmer where we use one, making its raw reported numbers not comparable to ours. Instead, \url{scripts/extract_published_baseline_comparison.py} (included in the released artifact) recomputes all four metrics from the package's raw generation/ground-truth text pairs using this paper's own \texttt{compute\_corpus\_metrics} --- the identical BERTScore checkpoint (\texttt{roberta-large}, English, no baseline rescaling), identical sacrebleu-based BLEU (0--100 scale), identical stemmed ROUGE-1, and identical METEOR computation on both sides of Table~\ref{tab:published} --- so every number in it is directly comparable, not merely placed in the same table. (GLM-4.6 itself has no dedicated technical report at the time of writing; we cite its immediate predecessor GLM-4.5~\cite{glm45}, whose architecture GLM-4.6 is a documented incremental update over.)

Two things are worth stating plainly under this harmonized recomputation. First, this is a genuine external reference point our own re-implementations alone cannot provide: these are the package's own real reported generations from models spanning roughly three orders of magnitude in scale relative to our reproducible local backbone (Qwen2.5-1.5B-Instruct), not our estimate of how such models would perform. Second, the harmonized numbers give a more differentiated picture than an unharmonized comparison would suggest, and we report it as such rather than soften it. Our own naive baselines hold up reasonably well against the published models under the comparable strategy: \texttt{zero\_shot} (BERTScore 0.824, ROUGE-1 0.300, METEOR 0.252) sits inside the published \texttt{Baseline}-strategy range on BERTScore and METEOR and above all four published rows on ROUGE-1, despite Gemma3-4B alone being roughly $2.7\times$ larger than our backbone --- a useful sanity check that our own baselines' absolute numbers are not artificially depressed by backbone weakness. \texttt{cadence\_full}, however, is not: at $B=5$ it scores below every one of the eight published rows (both strategies, all four models) on all four metrics, including BERTScore (0.806 versus a published range of 0.822--0.856). We do not read this as evidence that CADENCE is a worse decision-making \emph{process}: the terseness asymmetry established in Section~\ref{sec:discussion} plausibly explains part of the BLEU/ROUGE-1/METEOR gap, since CADENCE's structured, solver-parseable output is deliberately shorter than either our own open-ended baselines or the package's elaborate, few-shot-style generations. But we note plainly that this gap also appears on BERTScore, the metric we ourselves argue is comparatively insensitive to length mismatch, so length alone is not a full explanation; we consider the far larger scale of the published models (up to two orders of magnitude larger than our 1.5B-parameter backbone) the more likely dominant factor, though $N=3$ is too small to disentangle scale from architecture from prompting elaborateness in this comparison alone. We report this as a genuine limitation surfaced by external comparison, not a result to soften: a substantially larger held-out sample and evaluation against a stronger local backbone are needed before any claim about CADENCE's absolute competitiveness against frontier-scale systems can be supported.
