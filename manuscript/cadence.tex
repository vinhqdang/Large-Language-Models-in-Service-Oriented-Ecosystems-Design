...
We evaluate against the ``Context Matters'' replication package (Zenodo, DOI:~10.5281/zenodo.18370195), derived from the same mining study that provides Stage~1's retrieval corpus~\cite{buchgeher2023adr}. After parsing, the corpus yields 6{,}173 individual ADR files across 883 repository folders (median 4, mean 6.76, maximum 129 files per repository, confirming the corpus's documented per-repository sparsity). Each folder carries an extraction-confidence status from the package's own \texttt{dataset\_index.json}; we restrict our held-out evaluation set to the 750 folders marked \texttt{Verified}, so that ground-truth quality is not itself a confound. Following this corpus's own upstream precedent --- its companion sub-study is explicitly titled ``ADR Generation from Titles'' --- we construct each held-out query as the record's title (standing in for a stated decision context) and treat the record's full body as ground truth, matching the task formulation the retrieval-only baseline below is itself drawn from.
...
Table~\ref{tab:pilot} reports a small-$N$ pilot run used to verify the full pipeline and evaluation harness end-to-end before committing to a larger run; we retain it here as a sanity-checked, fully real (non-simulated) data point, and report our larger-scale results in Table~\ref{tab:scaled}.

% TODO(scaled results): scripts/run_evaluation_scaled.py --- N=15 held-out
% items, two tactic_budget conditions (achievable=5, tight=2), k=3,
% max_rounds=2, max_repair_iterations=2 --- is running in the background as
% of this draft. Replace the placeholder table below with
% data/processed/evaluation_results_scaled.json once the run completes, and
% update the prose in this subsection and in Section~\ref{sec:discussion}
% to reference the larger-N numbers as the primary result, keeping Table
% \ref{tab:pilot} only as the pilot/sanity-check data point.

\begin{table}[t]
\caption{Scaled Evaluation Run ($N=15$, Two \texttt{tactic\_budget} Conditions) --- \emph{Placeholder, Pending Background Run}\label{tab:scaled}}
\centering
\begin{tabular}{@{}lcccc@{}}
\toprule
System & BERTScore F1 & BLEU & ROUGE-1 F1 & METEOR \\
\midrule
\multicolumn{5}{l}{\emph{[to be filled in from evaluation\_results\_scaled.json]}} \\
\bottomrule
\end{tabular}
\end{table}

The pilot run's \texttt{cadence\_full} constraint-satisfaction rate of 0\% is an expected consequence of its deliberately tight parameters (\texttt{tactic\_budget=4} while requiring coverage of all 5 quality attributes against a tactic catalog in which every tactic supports exactly one attribute), not a defect in Stage~3: this configuration is mathematically infeasible by construction, and Table~\ref{tab:scaled}'s achievable-budget condition ($B=5$) is designed to demonstrate \texttt{cadence\_full} reaching \texttt{is\_feasible=True} in practice, with the tight-budget condition ($B=2$) retained specifically to demonstrate graceful degradation on purpose rather than by accident.

Across both baselines and CADENCE, BERTScore F1 is nearly flat (0.801--0.819 in the pilot run) while ROUGE-1 and METEOR are noticeably lower for the two deliberation-based systems (\texttt{multiagent\_no\_solver} and \texttt{cadence\_full}) than for the two single-pass systems. We discuss why in Section~\ref{sec:discussion}.
...
Our held-out evaluation set is drawn from a single corpus (a mining study of open-source ADR adoption); results may not transfer to proprietary or non-open-source architectural practice, where decision drivers, review culture, and documentation discipline can differ substantially. The local LLM backbone (Qwen2.5-1.5B-Instruct) is small by current standards, chosen for reproducibility rather than peak capability; we expect stronger backbones to shift absolute numbers, though we have no reason to expect the qualitative pattern of stage-wise contribution to invert. Our tactic catalog (25 hand-curated tactics across 5 quality attributes) is deliberately small and each tactic supports exactly one attribute in our current encoding, which sets a hard ceiling on achievable coverage under a tight budget by construction; a larger or multiply-supporting tactic catalog would relax this ceiling and merits future work. Finally, our qualitative critique score (Stage~4) is itself LLM-generated, and while it is deliberately blended with a deterministic structural component rather than trusted alone, it is not a substitute for human expert judgment, which we did not collect for this submission.
...
% TODO(citation-verify): confirm exact title/author list/venue/year and DOI
% or arXiv ID for the "Context Matters" ADR-generation-from-titles paper
% (design spec cites arXiv:2604.03826) once the citation-research agent
% returns verified results.
Authors TBD, ``Context Matters: [title to be confirmed],'' 2026. arXiv:2604.03826 (pending verification).

\bibitem{maad2025}
% TODO(citation-verify): confirm exact title/author list/venue/year for
% MAAD (design spec cites arXiv:2507.21382, "forthcoming ACM TOSEM") once
% the citation-research agent returns verified results.
Authors TBD, ``MAAD: [title to be confirmed],'' to appear in \textit{ACM Trans. Softw. Eng. Methodol.} arXiv:2507.21382 (pending verification).
...
% TODO(citation-verify): confirm exact citation for the LLM-to-symbolic-solver
% translation work cited here (candidate: Pan et al., "Logic-LM") once the
% citation-research agent returns verified results.
Authors TBD, ``Logic-LM: [title to be confirmed],'' 2023 (pending verification).

\bibitem{olausson2023linc}
% TODO(citation-verify): confirm exact citation for the LLM-authored-proof
% verification-via-formal-compilation work cited here (candidate: Olausson
% et al., "LINC") once the citation-research agent returns verified results.
Authors TBD, ``LINC: [title to be confirmed],'' 2023 (pending verification).

\bibitem{duetal2023debate}
% TODO(citation-verify): confirm exact citation for the multi-agent LLM
% debate work cited here once the citation-research agent returns verified
% results.
Authors TBD, ``Improving Factuality and Reasoning in Language Models through Multiagent Debate,'' 2023 (pending verification).

\bibitem{madaan2023selfrefine}
% TODO(citation-verify): confirm exact citation for the LLM self-refinement
% work cited here (candidate: Madaan et al., "Self-Refine") once the
% citation-research agent returns verified results.
Authors TBD, ``Self-Refine: Iterative Refinement with Self-Feedback,'' 2023 (pending verification).

\bibitem{lewis2020rag}
% TODO(citation-verify): confirm exact citation for the retrieval-augmented
% generation work cited here (candidate: Lewis et al., "RAG") once the
% citation-research agent returns verified results.
Authors TBD, ``Retrieval-Augmented Generation for Knowledge-Intensive NLP Tasks,'' 2020 (pending verification).

\bibitem{bass2021saip}
% TODO(citation-verify): confirm exact current edition/ISBN for the
% architectural tactics catalog cited here (candidate: Bass, Clements,
% Kazman, "Software Architecture in Practice") once the citation-research
% agent returns verified results.
L.~Bass, P.~Clements, and R.~Kazman, \textit{Software Architecture in Practice}, 4th ed. Boston, MA, USA: Addison-Wesley, 2021 (edition/ISBN pending verification).
